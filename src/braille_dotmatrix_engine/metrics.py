...
from .runtime_validation import as_binary_matrix, as_float_matrix, require_unit_interval


def _same_shape(reference, estimate):
    a = as_float_matrix(reference, name='reference')
    b = as_float_matrix(estimate, name='estimate')
    if a.shape != b.shape:
...
def occupancy(binary) -> dict:
    ...
    total = int(b.size)
    return {'active': active, 'total': total, 'ratio': float(active / max(1, total))}
...
def local_density_over(binary, limit: float) -> float:
    b = as_binary_matrix(binary).astype(np.float32)
    limit = require_unit_interval('limit', limit)
    density = uniform_filter(b, size=7, mode='reflect')
    return float(np.mean(density > limit))


def compute_quality_metrics(values, binary, cfg) -> dict:
    b = as_binary_matrix(binary, cfg).astype(np.float32)
